...
//! // contract:
//! let _guard = Guard::reentrancy_protect(&env, &ReentrancyKey::Book)?;
//!
...
pub enum ReentrancyKey {
    /// `book_workspace` reentrancy lock.
    Book,
    /// `cancel_booking` reentrancy lock.
    Cancel,
}
...
fn lock_symbol(env: &Env, key: &ReentrancyKey) -> Symbol {
    match key {
        ReentrancyKey::Book => Symbol::new(env, "lock_book"),
        ReentrancyKey::Cancel => Symbol::new(env, "lock_cancel"),
    }
}
